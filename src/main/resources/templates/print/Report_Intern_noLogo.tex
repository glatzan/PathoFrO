...
\ThisCenterWallPaper{1}{_background_noLogo.pdf}
%
\input{_code128.tex}
...
\noindent 
\textbf{$!subject} \\\\
$latexTextConverter.convertToTex($address)  \\
	...
	\begin{tabular}{p{0.47\linewidth}p{0.53\linewidth}}
		...
		& 
		{
			...
			\begin{tabular}[t]{p{0.38\linewidth}p{0.62\linewidth-8\tabcolsep}}
				...
				Eingangsdatum &  $!date.format('dd.MM.yyyy', $!task.receiptDate) \\
				...
				#set($externalSurgeon = $taskTool.getPrimaryContactFromString($task, "EXTERNAL_SURGEON"))
				#if($externalSurgeon)
				Einsender & $!externalSurgeon.person.getFullName() \\
				#end

				#set($surgeon = $taskTool.getPrimaryContactFromString($task, "SURGEON"))
				#if($surgeon)
				Operateur & $!surgeon.person.getFullName() \\
				#end

				#set($privatePhysician = $taskTool.getPrimaryContactFromString($task, "PRIVATE_PHYSICIAN"))
				#if($privatePhysician)
				Augenarzt & $!privatePhysician.person.getFullName() \\
				#end
