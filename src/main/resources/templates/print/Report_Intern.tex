	...
	\begin{tabular}{p{0.47\linewidth}p{0.53\linewidth}}
		...
		&
		{
			...
			\begin{tabular}[t]{p{0.38\linewidth}p{0.62\linewidth-8\tabcolsep}}
				...
				#set($externalSurgeon = $taskTool.getPrimaryContactFromString("EXTERNAL_SURGEON"))
				#if($externalSurgeon)
					Einsender & $!externalSurgeon.person.fullName \\
				#end

				#set($surgeon = $taskTool.getPrimaryContactFromString("SURGEON"))
				#if($surgeon)
					Operateur & $!surgeon.person.fullName \\
				#end

				#set($privatePhysician = $taskTool.getPrimaryContactFromString("PRIVATE_PHYSICIAN"))
				#if($privatePhysician)
					Augenarzt & $!privatePhysician.person.fullName \\
				#end
