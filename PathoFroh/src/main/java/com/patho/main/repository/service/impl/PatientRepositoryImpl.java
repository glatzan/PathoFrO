...
import com.patho.main.model.audit.Audit_;
...
public class PatientRepositoryImpl extends AbstractRepositoryCustom implements PatientRepositoryCustom {
	...
	public List<Patient> findByDateAndCriterion(FindCriterion findCriterion, long startDate, long endDate,
		...
		case StainingComplteted:
			predicates.add(builder.ge(taskQuery.get(Task_.stainingCompletionDate), startDate));
			predicates.add(builder.le(taskQuery.get(Task_.stainingCompletionDate), endDate));
			break;
		case DiagnosisCompleted:
			predicates.add(builder.ge(taskQuery.get(Task_.diagnosisCompletionDate), startDate));
			predicates.add(builder.le(taskQuery.get(Task_.diagnosisCompletionDate), endDate));
			break;
		case NotificationComplteted:
			predicates.add(builder.ge(taskQuery.get(Task_.notificationCompletionDate), startDate));
			predicates.add(builder.le(taskQuery.get(Task_.notificationCompletionDate), endDate));
			break;
		case TaskCompleted:
			predicates.add(builder.ge(taskQuery.get(Task_.finalizationDate), startDate));
			predicates.add(builder.le(taskQuery.get(Task_.finalizationDate), endDate));
			break;
		case TaskCreated:
