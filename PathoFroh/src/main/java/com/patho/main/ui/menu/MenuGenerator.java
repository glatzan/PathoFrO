...
public class MenuGenerator {
    ...
    public MenuModel generateEditMenu(Patient patient, Task task, HtmlPanelGroup taskMenuCommandButtons) {
        ...
        if (patient != null && userService.userHasPermission(HistoPermissions.TASK_EDIT)) {
            ...
            boolean taskIsEditable = taskIsNull ? false : task.getTaskStatus().getEditable();
            ...
            if (!taskIsNull) {
                ...
                // ***************************************************
                DefaultSeparator seperator = new DefaultSeparator();
                seperator.setRendered(task.getTaskStatus().getEditable());
                ...
                item.setRendered(task.getTaskStatus().getListStatus().getInListStaining() || task.getTaskStatus().getListStatus().getInListReStaining());
                item.setDisabled(!taskIsEditable);
                stainingSubMenu.addElement(item);
                ...
                item.setCommand(
                        "#{globalEditViewHandler.removeTaskFromFavouriteList(worklistHandler.selectedTask, "
                                + PredefinedFavouriteList.StayInStainingList.getId() + ")}");
                item.setIcon("fa fa-image");
                item.setRendered(task.getTaskStatus().getListStatus().getInListStayInStaining());
                ...
                item.setIcon("fa fa-image");
                item.setRendered(!(task.getTaskStatus().getListStatus().getInListStaining() || task.getTaskStatus().getListStatus().getInListReStaining()
                        || task.getTaskStatus().getListStatus().getInListStayInStaining()) && task.getTaskStatus().getEditable());
                item.setOncomplete("updateAndAutoScrollToSelectedElement('navigationForm:patientNavigationScroll')");
                item.setUpdate("navigationForm:patientList contentForm headerForm");
                stainingSubMenu.addElement(item);
            }
            ...
            if (!taskIsNull) {
                ...
                item.setRendered(
                        task.getTaskStatus().getListStatus().getInListDiagnosis() || task.getTaskStatus().getListStatus().getInListReDiagnosis());
                item.setDisabled(!taskIsEditable);
                diagnosisSubMenu.addElement(item);
                ...
                item.setRendered(task.getTaskStatus().getListStatus().getInListStayInDiagnosis());
                item.setOncomplete("updateAndAutoScrollToSelectedElement('navigationForm:patientNavigationScroll')");
                item.setUpdate("navigationForm:patientList contentForm headerForm");
                item.setIcon("fa fa-eye-slash");
                diagnosisSubMenu.addElement(item);
                ...
                item.setRendered(
                        !(task.getTaskStatus().getListStatus().getInListDiagnosis() || task.getTaskStatus().getListStatus().getInListReDiagnosis()
                                || task.getTaskStatus().getListStatus().getInListStayInDiagnosis()) && task.getTaskStatus().getEditable());
                item.setOncomplete("updateAndAutoScrollToSelectedElement('navigationForm:patientNavigationScroll')");
                item.setUpdate("navigationForm:patientList contentForm headerForm");
            ...
            if (!taskIsNull) {
                ...
                item.setRendered(!task.getTaskStatus().getFinalized());
                notificationSubMenu.addElement(item);
                ...
                item.setRendered(
                        task.getTaskStatus().getListStatus().getInListNotification() || task.getTaskStatus().getListStatus().getInListStayInNotification());
                notificationSubMenu.addElement(item);
                ...
                item.setUpdate("navigationForm:patientList contentForm headerForm");
                item.setIcon("fa fa-volume-off");
                item.setRendered(task.getTaskStatus().getListStatus().getInListStayInNotification());
                notificationSubMenu.addElement(item);

                // add to notification phase
                item = new DefaultMenuItem(resourceBundle.get("header.menu.task.sample.notification.enter"));
                item.setCommand(
                        "#{globalEditViewHandler.addTaskToFavouriteList(worklistHandler.selectedTask, "
                                + PredefinedFavouriteList.NotificationList.getId() + ")}");
                item.setOncomplete("updateAndAutoScrollToSelectedElement('navigationForm:patientNavigationScroll')");
                item.setUpdate("navigationForm:patientList contentForm headerForm");
                item.setIcon("fa fa-volume-off");
                item.setRendered(!(task.getTaskStatus().getListStatus().getInListNotification()
                        || task.getTaskStatus().getListStatus().getInListStayInNotification()) && task.getTaskStatus().getEditable());
                notificationSubMenu.addElement(item);
